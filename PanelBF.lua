...
do
	...
	G.Name, G.ResetOnSpawn, G.IgnoreGuiInset, G.DisplayOrder =
		"PanelLoad", false, true, 999
	...
	F("UIPadding", card, {
		...
		PaddingTop    = UDim.new(0, 22), PaddingBottom = UDim.new(0, 18),
	})
	...
	F("TextLabel", hdr, { Size = UDim2.new(0,60,1,0), Position = UDim2.new(1,-60,0,0), BackgroundTransparency=1, Font=Enum.Font.Gotham, TextSize=10, TextColor3=Color3.fromRGB(68,68,68), Text="Improved", TextXAlignment=Enum.TextXAlignment.Right, ZIndex=102 })
	...
	local sN = { "Waiting for game", "Game ready", "Workspace ready", "Leaderstats", "Character ready", "Enemies", "Building GUI", "Done" }
	local sL = {}
	...
	F("TextLabel", footer, { Size=UDim2.new(0,100,1,0), BackgroundTransparency=1, Font=Enum.Font.Gotham, TextSize=10, TextColor3=Color3.fromRGB(60,60,60), Text="v3 • Improved", TextXAlignment=Enum.TextXAlignment.Left, ZIndex=102 })
	...
	_closeLoader = function()
		...
		TS:Create(card, TweenInfo.new(.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
			Position = UDim2.new(.5, -170, .6, -100),
...
local cfg = {
	...
	HealthFPSThreshold = 30,
	HealthLagDuration  = 10,
...
local C = {
	...
	HOV   = Color3.fromRGB(28,28,28),
	SEP   = Color3.fromRGB(40,40,40),
	BOR   = Color3.fromRGB(55,55,55),
	BOR2  = Color3.fromRGB(80,80,80),
	WHT   = Color3.fromRGB(200,200,200),
	OFF   = Color3.fromRGB(185,185,185),
	MUT   = Color3.fromRGB(130,130,130),
	DIM   = Color3.fromRGB(95,95,95),
	OK    = Color3.fromRGB(70,155,90),
	WRN   = Color3.fromRGB(185,145,50),
	ERR   = Color3.fromRGB(185,70,70),
	BELI  = Color3.fromRGB(65,155,90),
	FRAG  = Color3.fromRGB(125,65,185),
	HOP   = Color3.fromRGB(175,55,125),
	WH    = Color3.fromRGB(55,120,185),
	PULL  = Color3.fromRGB(185,65,65),
	V1    = Color3.fromRGB(50,130,185),
	V2    = Color3.fromRGB(185,135,40),
	V3    = Color3.fromRGB(185,65,145),
	TABON  = Color3.fromRGB(70,155,90),
...
function Conn.new()
	return setmetatable({ _list = {} }, Conn)
end
function Conn:add(c)
	if c and typeof(c) == "RBXScriptConnection" then
		self._list[#self._list + 1] = c
	end
	return c
end
...
local function wFmt(n)
	return (n < 0 and "-" or "+") ..
		tostring(math.floor(math.abs(n))):reverse():gsub("(%d%d%d)", "%1,"):reverse():gsub("^,", "")
end
...
local function fmtSpawn(s)
	...
	s = tostring(s):gsub("([a-z])([A-Z])", "%1 %2"):gsub("_", " ")
		:gsub("(%a)([%w]*)", function(f2, r) return f2:upper() .. r:lower() end)
	return s
end
local function pushH(t, v)
	if type(v) ~= "number" then return end
	t[#t+1] = { t = tick(), v = v }
	while #t > K.HMAX do table.remove(t, 1) end
end
local function calcRate(t)
	if #t < 2 then return 0 end
	local e = t[#t].t - t[1].t
	if e < 1 then return 0 end
	return math.floor((t[#t].v - t[1].v) / (e / 60))
end
...
local function stopFakeLevel()
	...
	local obj = getRealLevelObj()
	if obj and _realLevel then pcall(function() obj.Value = _realLevel end) end
end
...
local function startFakeLevel(targetVal)
	...
	S.fakeLevel    = true
	S.fakeLevelVal = targetVal
	S.fakeLevelThread = task.spawn(function()
		local conns = Conn.new()
		local fns = {
			function() return lp.Data.Level end,
			function() return lp.leaderstats.Level end,
			function() return lp.leaderstats.Lv end,
		...
		for _, fn in ipairs(fns) do
			...
			if ok and o and o:IsA("ValueBase") then
				conns:add(o.Changed:Connect(function(v)
					if S.fakeLevel and v ~= targetVal then
		...
		conns:add(lp.CharacterAdded:Connect(function()
			...
			local no = getRealLevelObj()
			if no then pcall(function() no.Value = targetVal end) end
		end))
...
local function startSpec(p)
	...
	S.specCharConn = p.CharacterAdded:Connect(function(char)
		task.wait(.5)
		if S.specTarget == p then attachCamera(char) end
	end)
	...
	S.specConn = Run.RenderStepped:Connect(function()
		...
		for _, pl in ipairs(Pl:GetPlayers()) do
			if pl == S.specTarget then found = true; break end
		end
...
local function showN(name, sub, col)
	if #activeNotifs >= NMAX then
		...
		for _, c in ipairs(old.frame:GetDescendants()) do
			pcall(function()
				...
				elseif c:IsA("UIStroke")  then tw(c, { Transparency = 1 }, .2)
				end
			end)
		end
	...
	local f = mk("Frame", gui, {
		Size = UDim2.new(0, NW, 0, NH),
		Position = UDim2.new(1, NW+10, 0, 60),
		BackgroundColor3 = C.PAN,
		BackgroundTransparency = 1,
		ZIndex = 60,
	})
	stroke(f, C.BOR2, 1); corner(f, 6)

	local dot2 = mk("Frame", f, { Size = UDim2.new(0,8,0,8), Position = UDim2.new(0,10,0,10), BackgroundColor3 = col or C.OK, ZIndex = 61 })
	corner(dot2, 4)
	...
	tw(dot2, { BackgroundTransparency = 0 }, .2)
	...
	task.delay(3, function()
		...
		tw(nLbl, { TextTransparency = 1 }, .25)
		tw(sLbl, { TextTransparency = 1 }, .25)
		tw(dot2, { BackgroundTransparency = 1 }, .25)
			...
			for i, v in ipairs(list) do
				pcall(function()
					if v:IsA("BasePart") and not v:IsDescendantOf(lp.Character or {}) then
						S.v1Parts[#S.v1Parts+1] = { o = v, t = v.Transparency }
						v.Transparency = 1
					end
				end)
		...
		task.spawn(function()
			for i, d in ipairs(S.v1Parts) do
				if d.o and d.o.Parent then
					pcall(function() d.o.Transparency = d.t end)
				end
				if i % 200 == 0 then task.wait() end
			end
			S.v1Parts = {}
...
local function setV2(on)
	...
	if on then
		...
		L.Ambient = Color3.new(.5,.5,.5); L.OutdoorAmbient = Color3.new(.5,.5,.5)
		L.ClockTime = 14
		...
		if ter then
			S.v2Orig.WW  = ter.WaterWaveSize;  S.v2Orig.WS2 = ter.WaterWaveSpeed
			ter.WaterWaveSize = 0; ter.WaterWaveSpeed = 0
			ter.WaterReflectance = 0; ter.WaterTransparency = 1
		end
		...
		local function stripObj(o)
			if o:IsA("ParticleEmitter") or o:IsA("Trail") or o:IsA("Smoke") or o:IsA("Fire") or o:IsA("Sparkles") then
				o.Enabled = false; if o.Rate ~= nil then o.Rate = 0 end
		...
		S.v2Conn = game.DescendantAdded:Connect(function(o)
			if S.v2 then task.defer(function() pcall(stripObj, o) end) end
		end)
		...
		local ter = WS:FindFirstChildOfClass("Terrain")
		if ter and o.WW ~= nil then
			ter.WaterWaveSize = o.WW; ter.WaterWaveSpeed = o.WS2
		end
		for _, c in ipairs(L:GetChildren()) do
...
local function stripCharCosmetics(char)
	...
	for _, obj in ipairs(char:GetDescendants()) do
		pcall(function()
			...
			elseif obj:IsA("Beam") or obj:IsA("PointLight") or obj:IsA("SpotLight") then
				obj.Enabled = false
			end
		end)
	end
...
local function stripVisualObj(o)
	...
	elseif o:IsA("ParticleEmitter") or o:IsA("Trail") or o:IsA("Fire") or o:IsA("Sparkles") or o:IsA("Smoke") then
		o.Enabled = false; if o:IsA("ParticleEmitter") then o.Rate = 0 end
	elseif o:IsA("Beam") or o:IsA("PointLight") or o:IsA("SpotLight") or o:IsA("SurfaceLight") then
		o.Enabled = false
	elseif o:IsA("BillboardGui") or o:IsA("SurfaceGui") then
		o.Enabled = false
	elseif o:IsA("Sound") then
		if o.Name:lower():find("ambient") or o.Name:lower():find("music") then o.Volume = 0 end
	end
end
...
local function setV3(on)
	if on then
		...
		S.v3Conns:add(WS.DescendantAdded:Connect(function(o)
			if not S.v3 then return end
			task.defer(function() pcall(stripVisualObj, o) end)
		end))

		S.v3Conns:add(lp.CharacterAdded:Connect(function(char)
			...
			S.v3Conns:add(char.ChildAdded:Connect(function(child)
				...
				pcall(function()
					if child:IsA("Accessory") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") then
						task.wait(.1); if S.v3 then child:Destroy() end
					end
				end)
			end))
		end

		S.v3Conns:add(lp.CharacterAdded:Connect(watchCharChildren))
		if lp.Character then watchCharChildren(lp.Character) end
	else
		S.v3Conns:disconnectAll()
	end
end
...
local function bmClean()
	for e in pairs(BM.data) do pcall(bmRelease, e) end
	BM.data = {}
end

local function bmGetOff()
	local a = math.random() * math.pi * 2
	local r = math.random(2, 5)
	return Vector3.new(math.cos(a)*r, 0, math.sin(a)*r)
end

local function bmMyRoot()
	...
	BM.pin = Run.Heartbeat:Connect(function()
		...
		for e, d in pairs(BM.data) do
			...
			if d.bp and d.bp.Parent then
				pcall(function() d.bp.Position = Vector3.new(d.bp.Position.X, mr.Position.Y + BM.yOff, d.bp.Position.Z) end)
			end
...
local function addLog(source)
	local b  = getStat("Beli")      or 0
	local f  = getStat("Fragments") or 0
	...
	table.insert(S.logs, 1, {
		...
		bPM     = math.floor(sB/minIn), fPM = math.floor(sF/minIn),
	})
...
local function _sendWH(url, payload)
	...
	if not ok or type(json) ~= "string" then
		warn("[WH] encode fail: " .. tostring(json)); return
	end
	...
	if not ok2 then warn("[WH] request error: " .. tostring(res)); return end
	if type(res) == "table" and res.StatusCode ~= 204 then
		warn("[WH] Status=" .. tostring(res.StatusCode) .. " Body=" .. tostring(res.Body))
	end
end
...
local function getEquipped()
	local c = lp.Character; if not c then return "None", nil end
	for _, o in ipairs(c:GetChildren()) do
		if o:IsA("Tool") then return o.Name, getToolLv(o) end
	end
	return "None", nil
end
...
local function getInv()
	local items = {}
	local bp = lp:FindFirstChild("Backpack"); if not bp then return items end
	for _, o in ipairs(bp:GetChildren()) do
		if o:IsA("Tool") and o.Name ~= "Tool" then
			local lv = getToolLv(o)
			if lv ~= nil then items[#items+1] = { name=o.Name, level=lv } end
		end
	end
	return items
end
...
local function getSkillLevels(name)
	...
	pcall(function()
		...
		for _, child in ipairs(iF:GetChildren()) do
			...
			local lo = child:FindFirstChild("Level"); if not lo then continue end
			local v
			if lo:IsA("TextLabel") or lo:IsA("TextButton") then
				v = tonumber(lo.Text:match("%d+"))
			elseif lo:IsA("IntValue") or lo:IsA("NumberValue") then
				v = lo.Value
			end
...
local function sendMilestoneWebhook(statName, target, totalBeli, totalFrag)
	...
	local jt    = S.plrC[lp.UserId] and S.plrC[lp.UserId].join or tick()
	local bPM2  = calcRate(S.beliHist)
	local cur   = statName == "Beli" and totalBeli or totalFrag
	local bar   = emojiBar(math.min(cur, target), target)
	_sendWH(url, {
		username = cfg.WebhookName or "BloxHub",
		embeds = {{ title="🏆 Milestone — "..fmtN(target).." "..statName,
			description = "**["..pName.."](https://www.roblox.com/users/"..lp.UserId.."/profile)**\nReached **"..fmtN(target).."** "..statName.."!",
			color = statName == "Beli" and 3447003 or 10181046,
			fields = {
				buildField("Target",     "```"..fmtN(target).."```",        true),
				buildField("Total Beli", "```"..fmtN(totalBeli).."```",      true),
				buildField("Total Frags","```"..fmtN(totalFrag).."```",      true),
				buildField("Session",    "```"..fmtS(tick()-jt).."```",      true),
				buildField("Beli/min",   "```"..wFmt(bPM2).."```",           true),
				buildField("Progress",   bar,                                false),
			},
			footer = { text = "BloxHub v3 • Milestone Report" }, timestamp = ts(),
		}},
	})
end
...
local function sendHealthWebhook(avgFPS, instanceCount, elapsed)
	...
	_sendWH(url, {
		username = cfg.WebhookName or "BloxHub",
		embeds = {{ title="⚠️ Server Health Alert",
			description = "**["..pName.."](https://www.roblox.com/users/"..lp.UserId.."/profile)**\nServer lag detected!",
			color = 15158332,
			fields = {
				buildField("Avg FPS",    "```"..avgFPS.."```",                                    true),
				buildField("Instances",  "```"..fmtN(instanceCount).."```",                       true),
				buildField("Ping",       "```"..getPing().."ms```",                                true),
				buildField("Session",    "```"..fmtS(elapsed).."```",                             true),
				buildField("Players",    "```"..#Pl:GetPlayers().."/"..K.MAX.."```",              true),
				buildField("FPS Gauge",  emojiBar(avgFPS, 60),                                    false),
			},
			footer = { text = "BloxHub v3 • Server Health" }, timestamp = ts(),
		}},
	})
end
...
local function sendWebhook(sessBeli, sessFrags, elapsed, source)
	...
	def=def or 0; fruit=fruit or 0; bounty=bounty or 0; spawn2=spawn2 or "Unknown"
	...
	pcall(function()
		if lp.DisplayName ~= lp.Name then pName = lp.DisplayName .. " (@" .. lp.Name .. ")" end
	end)

	local embedColor = bPM2 >= cfg.PerfBaselineBPM and 3066993
		or bPM2 >= cfg.PerfBaselineBPM * 0.5 and 16776960 or 15158332

	local eqStr = "Nothing"
	pcall(function()
		for _, o in ipairs((lp.Character or {}):GetChildren()) do
			if o:IsA("Tool") then
				local lv2 = getToolLv(o)
				eqStr = o.Name .. (lv2 and " LV"..fmtN(lv2) or ""); break
			end
		end
	end)
	...
	pcall(function()
		...
		if bp then
			for _, o in ipairs(bp:GetChildren()) do
				if o:IsA("Tool") and o.Name ~= "Tool" and #invLines < 8 then
					local lv2 = getToolLv(o)
					invLines[#invLines+1] = o.Name .. (lv2 and " LV"..fmtN(math.floor(lv2)) or "")
				end
			end
		end
	end)
	...
	pcall(function()
		for _, p in ipairs(Pl:GetPlayers()) do
			...
			plrLines[#plrLines+1] = tostring(p.Name)
	...
	local bPMStr    = (bPM2 >= 0 and "+" or "") .. fmtN(bPM2)
	local fPMStr    = (fPM2 >= 0 and "+" or "") .. fmtN(fPM2)

	local fields = {
		buildField("Player",       pName,                                     true),
		buildField("Level",        tostring(lv),                              true),
		buildField("Bounty",       fmtN(bounty),                              true),
		buildField("Total Beli",   fmtN(beli),                                true),
		buildField("Total Frags",  fmtN(frag),                                true),
		buildField("Beli Gained",  sessBStr,                                  true),
		buildField("Frags Gained", sessFS,                                    true),
		buildField("Session",      fmtS(elapsed or 0),                       true),
		buildField("Beli/Min",     bPMStr,                                    true),
		buildField("Beli/Hr",      (bPM2 >= 0 and "+" or "") .. fmtN(bPM2*60), true),
		buildField("Frag/Min",     fPMStr,                                    true),
		buildField("Melee",        fmtN(melee),                               true),
		buildField("Sword",        fmtN(sword),                               true),
		buildField("Gun",          fmtN(gun),                                 true),
		buildField("Defense",      fmtN(def),                                 true),
		buildField("Blox Fruit",   fmtN(fruit),                               true),
		buildField("Equipped",     eqStr,                                     true),
		buildField("Spawn",        spawn2,                                    true),
		buildField("Players",      tostring(#Pl:GetPlayers()).."/"..K.MAX,    true),
		buildField("FPS / Ping",   S.fps.." FPS | "..getPing().."ms",         true),
		buildField("Backpack",     #invLines > 0 and table.concat(invLines, ", ") or "-", false),
		buildField("Player List",  #plrLines > 0 and table.concat(plrLines, "\n") or "-", false),
		buildField("Server Health",healthStr,                                 false),
	}
	...
	_sendWH(url, {
		...
		embeds = {{
			...
			description = pName .. " | Session: " .. fmtS(elapsed or 0)
				.. "\nBeli " .. sessBStr .. " ("..bPMStr.."/min)"
				.. "\nFrags " .. sessFS  .. " ("..fPMStr.."/min)",
			fields  = fields,
			footer  = { text = "BloxHub v3 | Report #"..S.whTotal },
			timestamp = ts(),
		}},
	})
end
			...
			if S.whCD <= 0 then
				...
				if S.whTimer and cfg.WebhookEnabled then
					task.spawn(function()
						...
						local jt  = S.plrC[lp.UserId] and S.plrC[lp.UserId].join or tick()
						sendWebhook(
							S.sessOK and math.floor(cb - (S.sessB or cb)) or 0,
							S.sessOK and math.floor(cf - (S.sessF or cf)) or 0,
							tick() - jt, "Webhook Time"
						)
					end)
				end
			end
...
local function stopWHTimer()
	S.whTimer = false
	if S.whThread then task.cancel(S.whThread); S.whThread = nil end
	S.whCD = cfg.WebhookInterval * 60
end
...
local function doHop()
	...
	local watchThread = task.spawn(function()
		while watching do
			task.wait(.5)
			if not sb.Enabled or not frame.Visible then ensureOpen() end
		end
	end)
	...
	local function findBest()
		...
		local function scrollDown() GS:ReducedMotionEnabled();  -- no-op guard
		...
		local maxRefresh = 3
		for pass = 1, maxRefresh do
			for _ = 1, 30 do scrollUp() end
			task.wait(.5); seenJobs = {}; readCurrentRows()

			local noNew = 0
			while noNew < 10 do
				for _ = 1, 3 do scrollDown() end
				task.wait(.25)
				if readCurrentRows() then noNew = 0 else noNew = noNew + 1 end
				if best then showN("Auto Hop", "Found! "..bestC.." players", C.HOP); return best end
			end

			if pass < maxRefresh then
				showN("Auto Hop", "No server ≤"..maxP.." players, refreshing... ("..pass.."/"..maxRefresh..")", C.WRN)
				tried = {}
				pcall(function() frame.Refresh:Activate() end)
				task.wait(4)
			end
	...
	local function tryHop()
		...
		if server then
			...
			showN("Auto Hop", "No server found after all retries", C.ERR)
		end
	end
...
local function startHop()
	...
	S.hopThread = task.spawn(function()
		while S.hop do
			task.wait(1)
			local now = tick()
			S.hopCD = S.hopCD - (now - S.hopTick)
			S.hopTick = now
			if S.hopCD <= 0 then
				S.hopCD = cfg.HopInterval * 60
				if S.hop then task.spawn(doHop) end
			end
			S.dirty.fast = true
		end
	end)
...
local function stopHop()
	S.hop = false
	if S.hopThread then task.cancel(S.hopThread); S.hopThread = nil end
	S.hopCD = cfg.HopInterval * 60
	pcall(function()
		local sb = pg:FindFirstChild("ServerBrowser"); if not sb then return end
		sb.Enabled = false
		local f = sb:FindFirstChild("Frame"); if f then f.Visible = false end
	end)
end
...
local function checkServerHealth()
	...
	S.health.lastCheck = now

	table.insert(S.health.fpsHist, S.fps)
	if #S.health.fpsHist > 30 then table.remove(S.health.fpsHist, 1) end
	local sum = 0
	for _, v in ipairs(S.health.fpsHist) do sum = sum + v end
	...
	if S.health.avgFPS < threshold then
		if not S.health.lagDetected then
			S.health.lagDetected = true; S.health.lagTick = now
		elseif (now - S.health.lagTick) >= (cfg.HealthLagDuration or 10) and not S.health.reportSent then
			S.health.reportSent = true
			addEvent("⚠️ Server Lag", "Avg FPS: "..S.health.avgFPS.." | Max: "..S.maxFps.." | Parts: "..fmtN(S.health.instanceCount), C.ERR)
		...
		S.health.lagDetected = false; S.health.reportSent = false
	end
	S.dirty.health = true
end
...
local function checkMilestones()
	local b = getStat("Beli") or 0
	local f = getStat("Fragments") or 0
	for _, target in ipairs(cfg.MilestoneBeli or {}) do
		if b >= target and not S.milestone.beliReached[target] then
			S.milestone.beliReached[target] = true
			addEvent("💰 Beli Milestone", fmtN(target).." Beli!", C.BELI)
			showN("Milestone!", fmtN(target).." Beli reached!", C.BELI)
			if cfg.WebhookEnabled then task.spawn(sendMilestoneWebhook, "Beli", target, b, f) end
		end
	end
	for _, target in ipairs(cfg.MilestoneFrag or {}) do
		if f >= target and not S.milestone.fragReached[target] then
			S.milestone.fragReached[target] = true
			addEvent("💎 Frag Milestone", fmtN(target).." Frags!", C.FRAG)
			showN("Milestone!", fmtN(target).." Frags reached!", C.FRAG)
			if cfg.WebhookEnabled then task.spawn(sendMilestoneWebhook, "Fragments", target, b, f) end
		end
	end
end
...
local function watchPlr(p)
	if p == lp then return end
	local uid = p.UserId
	S.plrC[uid] = S.plrC[uid] or { join = tick() }

	task.spawn(function()
		local d = p:FindFirstChild("Data") or p:WaitForChild("Data", 30)
		if not d then return end

		local function watchVal(childName, key)
			local obj = d:FindFirstChild(childName) or d:WaitForChild(childName, 30)
			if not obj then return end
			local function upd(v) S.plrC[uid] = S.plrC[uid] or {}; S.plrC[uid][key] = v end
			upd(obj.Value)
			if S.spawnW[uid] then S.spawnW[uid]:Disconnect() end
			S.spawnW[uid] = obj.Changed:Connect(upd)
		end
		watchVal("LastSpawnPoint", "spawn")
		...
		if rc then
			...
			S.raceW[uid] = rc.Changed:Connect(function(v)
				S.plrC[uid] = S.plrC[uid] or {}
				if v ~= "" then S.plrC[uid].race = tostring(v) end
			end)
		end
	end)
	...
	task.spawn(function()
		...
		S.bountyW[uid] = bObj.Changed:Connect(function(v)
			S.plrC[uid] = S.plrC[uid] or {}; S.plrC[uid].bounty = v
		end)
	end)
end
...
local function lbl(par, p)
	return mk("TextLabel", par, {
		BackgroundTransparency = 1,
		Font              = p.font or Enum.Font.GothamBold,
		TextSize          = p.sz   or 13,
		TextColor3        = p.col  or C.OFF,
		Text              = p.txt  or "",
		Size              = p.size or UDim2.new(1,0,0,18),
		...
		TextTruncate      = p.tr   or Enum.TextTruncate.None,
		ZIndex            = p.z    or 2,
	})
end
...
local full   = mk("Frame", gui, {
	Size = UDim2.new(0,K.HW,0,K.HH), Position = hudPos,
	BackgroundColor3 = C.PAN, BorderSizePixel = 0, ClipsDescendants = true, ZIndex = 2,
})
...
for i, tab in ipairs(TABS) do
	...
	local tb = mk("TextButton", tabBar, {
		...
		Text=tab.label, TextColor3=C.DIM, TextSize=11, Font=Enum.Font.GothamBold,
...
local function switchTab(id)
	...
	if oldSF then
		...
		for _, obj in ipairs(oldSF:GetDescendants()) do
			pcall(function()
				if obj:IsA("TextLabel") or obj:IsA("TextButton") then
					TS:Create(obj, TweenInfo.new(.1), { TextTransparency = 1 }):Play()
				end
			end)
		end
	...
	if newSF then
		for _, obj in ipairs(newSF:GetDescendants()) do
			pcall(function()
		...
		task.delay(.05, function()
			...
			for _, obj in ipairs(newSF:GetDescendants()) do
				pcall(function()
					if obj:IsA("TextLabel") or obj:IsA("TextButton") then
						TS:Create(obj, TweenInfo.new(.15), { TextTransparency = 0 }):Play()
					end
				end)
			end
		end)
...
do
	...
	lbl(ecoRow1, { size=UDim2.new(0,hw2,0,12), sz=8, col=C.DIM, txt="BELI", z=5 })
	...
	rateCol(0,          "BELI/MIN",  C.BELI, "bPMLbl")
	rateCol(qw+2,       "BELI/HR",   C.BELI, "bHRLbl")
	rateCol((qw+2)*2,   "FRAG/MIN",  C.FRAG, "fPMLbl")
...
do
	...
	local capBox = inlineBox(fpsRow, 0, K.IW-64, cfg.LockFps.fps.." FPS")
	...
	local function onFpsSet()
		...
		if n and n > 0 then applyFPS(n); capBox.Text = ""; capBox.PlaceholderText = n.." FPS"; showN("FPS Lock","Set to "..n.." FPS",C.OK) end
	end
	...
	applyBtn.MouseButton1Click:Connect(function()
		...
		cfg.WebhookURL  = url
		cfg.WebhookName = whNameBox.Text ~= "" and whNameBox.Text or "BloxHub"
		setText(UI.whApplyStatus, "Saved — "..cfg.WebhookName); setCol(UI.whApplyStatus, C.OK)
		showN("Webhook","URL saved",C.WH)
	end)
	...
	UI.rerunBtn.MouseButton1Click:Connect(function()
		if S.rerun then stopRerun();   tog(UI.rerunBtn, false, C.RERUN, C.BTN_OFF, "Auto Rerun: On", "Auto Rerun: Off")
		else           startRerun();   tog(UI.rerunBtn, true,  C.RERUN, C.BTN_OFF, "Auto Rerun: On", "Auto Rerun: Off") end
	end)

	local sec7 = section("controls", 7, "Fake Level")
	secLbl(sec7, 2, "Replaces .Value on .Changed (client-side only)", C.DIM, 9)
	...
	UI.fakeLvBtn.MouseButton1Click:Connect(function()
		if S.fakeLevel then
			stopFakeLevel()
			tog(UI.fakeLvBtn, false, C.FAKE, C.BTN_OFF, "Fake Level: On", "Fake Level: Off")
			setText(UI.fakeLvStatus, "OFF"); setCol(UI.fakeLvStatus, C.DIM)
			showN("Fake Level","Disabled",C.ERR)
		else
			local n = tonumber(fakeLvBox.Text)
			if not n or n <= 0 then showN("Fake Level","Enter a level first!",C.WRN); return end
			startFakeLevel(n)
			tog(UI.fakeLvBtn, true, C.FAKE, C.BTN_OFF, "Fake Level: On", "Fake Level: Off")
			setText(UI.fakeLvStatus, "ACTIVE — LV "..fmtN(n)); setCol(UI.fakeLvStatus, C.FAKE)
...
do
	...
	bm2XBox = inlineBox(xyzRow, 0, xyzW, "X")
	bm2YBox = inlineBox(xyzRow, xyzW+4, xyzW, "Y")
	bm2ZBox = inlineBox(xyzRow, (xyzW+4)*2, xyzW, "Z")
	...
	setYBtn.MouseButton1Click:Connect(function()
		...
		if n ~= nil then BM.yOff=n; yOffBox.Text=""; yOffBox.PlaceholderText="Y: "..n; setText(yHdr,"Y Offset  [current: "..n.."]  (V1 & V2)"); setText(UI.bmYLbl,"Y Offset (shared): "..n); S.last[yHdr]=nil; S.last[UI.bmYLbl]=nil; showN("BringMob","Y Offset → "..n,C.V1)
	...
	bm2AnchorBtn.MouseButton1Click:Connect(function()
		...
		if not hrp then showN("BringMob V2","No Character!",C.WRN); return end
...
do
	...
	for i = 1, 20 do
		...
		specBtn.MouseLeave:Connect(function()
			local p = UI.plrRowMap[idx]
			tw(specBtn, { BackgroundColor3 = (p and S.specTarget==p) and Color3.fromRGB(220,80,80) or C.SPEC }, .1)
		end)
	end
end
...
do
	...
	lbl(eqRow, { size=UDim2.new(1,0,0,12), sz=8, col=C.DIM, txt="EQUIPPED TOOL", z=5 })
	...
	UI.invRows = {}
	for i = 1, 20 do
		local cell = mk("Frame", invSF, { Size=UDim2.new(1,-4,0,58), BackgroundColor3=Color3.fromRGB(16,16,16), ZIndex=5, LayoutOrder=i, Visible=false })
		stroke(cell, C.BOR, 1); corner(cell, 4)
		local skillLbls = {}
...
do
	...
	local afkRowF = mk("Frame", afkSec, { Size=UDim2.new(1,0,0,44), BackgroundTransparency=1, LayoutOrder=3, ZIndex=4 })
	local afkW    = math.floor(K.IW/4) - 3
	for i, h in ipairs(afkHours) do
		local xi = (i-1)*(afkW+4)
		lbl(afkRowF, { size=UDim2.new(0,afkW,0,12), pos=UDim2.new(0,xi,0,0), sz=8, col=C.DIM, txt=h.."H", z=5 })
		UI.afkRows[i] = {
			bL = lbl(afkRowF, { size=UDim2.new(0,afkW,0,14), pos=UDim2.new(0,xi,0,12), sz=10, col=C.BELI, txt="+0", z=5 }),
			fL = lbl(afkRowF, { size=UDim2.new(0,afkW,0,13), pos=UDim2.new(0,xi,0,27), sz=9, col=C.FRAG, txt="+0", z=5 }),
			h  = h,
		}
	end
	...
	lbl(healthRow, { size=UDim2.new(0,hw3,0,12), sz=8, col=C.DIM, txt="AVG FPS", z=5 })
	lbl(healthRow, { size=UDim2.new(0,hw3,0,12), pos=UDim2.new(0,hw3+4,0,0), sz=8, col=C.DIM, txt="INSTANCES", z=5 })
	lbl(healthRow, { size=UDim2.new(0,hw3,0,12), pos=UDim2.new(0,(hw3+4)*2,0,0), sz=8, col=C.DIM, txt="STATUS", z=5 })
	UI.hFpsLbl  = lbl(healthRow, { size=UDim2.new(0,hw3,0,16), pos=UDim2.new(0,0,0,14), sz=13, col=C.OK, txt="60", z=5 })
	UI.hInstLbl = lbl(healthRow, { size=UDim2.new(0,hw3,0,16), pos=UDim2.new(0,hw3+4,0,14), sz=13, col=C.OFF, txt="0", z=5 })
	UI.hStatLbl = lbl(healthRow, { size=UDim2.new(0,hw3,0,16), pos=UDim2.new(0,(hw3+4)*2,0,14), sz=11, col=C.OK, txt="✅ OK", z=5 })
	...
	threshBtn.MouseButton1Click:Connect(function()
		...
		if n and n > 0 then cfg.HealthFPSThreshold=n; threshBox.Text=""; threshBox.PlaceholderText="Threshold: "..n; showN("Server Health","Alert FPS < "..n,C.OK)
	...
	local logSF = mk("ScrollingFrame", logListSec, { Size=UDim2.new(1,0,0,200), BackgroundTransparency=1, BorderSizePixel=0, ScrollBarThickness=3, ScrollBarImageColor3=C.BOR2, CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y, ClipsDescendants=true, ZIndex=4, LayoutOrder=2 })
	mk("UIListLayout", logSF, { Padding=UDim.new(0,4), SortOrder=Enum.SortOrder.LayoutOrder })
	UI.logRows = {}
	for i = 1, 50 do
		local card = mk("Frame", logSF, { Size=UDim2.new(1,-4,0,76), BackgroundColor3=Color3.fromRGB(14,14,14), ZIndex=5, LayoutOrder=i, Visible=false })
	...
	stroke(evClrBtn, C.BOR2, 1); corner(evClrBtn, 4); addHov(evClrBtn, function() return C.BTN_OFF end)
	local evSF = mk("ScrollingFrame", evSec, { Size=UDim2.new(1,0,0,240), BackgroundTransparency=1, BorderSizePixel=0, ScrollBarThickness=3, ScrollBarImageColor3=C.BOR2, CanvasSize=UDim2.new(0,0,0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y, ClipsDescendants=true, ZIndex=4, LayoutOrder=3 })
	mk("UIListLayout", evSF, { Padding=UDim.new(0,3), SortOrder=Enum.SortOrder.LayoutOrder })
	UI.evRows = {}
	for i = 1, 100 do
		local row = mk("Frame", evSF, { Size=UDim2.new(1,-4,0,36), BackgroundColor3=Color3.fromRGB(14,14,14), ZIndex=5, LayoutOrder=i, Visible=false })
		stroke(row, C.BOR, 1); corner(row, 4)
		mk("UIPadding",    row, { PaddingLeft=UDim.new(0,8), PaddingRight=UDim.new(0,8), PaddingTop=UDim.new(0,4), PaddingBottom=UDim.new(0,4) })
		mk("UIListLayout", row, { Padding=UDim.new(0,1), SortOrder=Enum.SortOrder.LayoutOrder })
		UI.evRows[i] = {
			row       = row,
			typeLbl   = mk("TextLabel", row, { Size=UDim2.new(1,0,0,13), BackgroundTransparency=1, Font=Enum.Font.GothamBold, TextSize=11, TextColor3=C.WHT, Text="", TextXAlignment=Enum.TextXAlignment.Left, LayoutOrder=1, ZIndex=6 }),
			detailLbl = mk("TextLabel", row, { Size=UDim2.new(1,0,0,11), BackgroundTransparency=1, Font=Enum.Font.Gotham, TextSize=9, TextColor3=C.DIM, Text="", TextXAlignment=Enum.TextXAlignment.Left, LayoutOrder=2, ZIndex=6 }),
		}
	end
	...
	local function refreshLogUI()
		...
		for i = 1, 50 do
			...
			local entry = S.logs[i]
			if entry then
				row.card.Visible = true
				local srcCol = srcColors[entry.source] or C.DIM
				row.line1.TextColor3 = srcCol
				setText(row.line1, "#"..i.."  ["..entry.source.."]  —  "..entry.time)
				setText(row.line2, "Beli  "..(entry.beli>=0 and"+" or"")..fmtN(entry.beli).."   ("..wFmt(entry.bPM).."/min  "..wFmt(entry.bPM*60).."/hr)")
				setText(row.line3, "Frag  "..(entry.frags>=0 and"+" or"")..fmtN(entry.frags).."   ("..wFmt(entry.fPM).."/min  "..wFmt(entry.fPM*60).."/hr)")
				setText(row.line4, "Duration: "..fmtS(entry.elapsed))
			else
	...
	local function refreshEventUI()
		for i = 1, 100 do
			local row   = UI.evRows[i]
			local entry = S.events[i]
			if entry then
				row.row.Visible = true
				row.typeLbl.TextColor3 = entry.col
				setText(row.typeLbl,   entry.evType)
				setText(row.detailLbl, entry.detail.."  —  "..entry.time)
			else
				row.row.Visible = false
			end
		end
	end
	local function refreshAfkUI()
		local bPM = calcRate(S.beliHist); local fPM = calcRate(S.fragHist)
		setText(UI.afkRateLbl, "Rate: "..wFmt(bPM).."/min Beli  "..wFmt(fPM).."/min Frag")
		for _, r in ipairs(UI.afkRows) do
			local mins = r.h * 60
			setText(r.bL, "+"..fmtN(math.max(0, math.floor(bPM*mins))))
			setText(r.fL, "+"..fmtN(math.max(0, math.floor(fPM*mins))))
		end
	end
	local function refreshHealthUI()
		...
		setText(UI.hFpsLbl,  "Avg "..tostring(avg).." / Max "..tostring(S.maxFps))
		...
		setBar(UI.hBarFl,    math.clamp(avg / math.max(S.maxFps, 1), 0, 1))
	...
	UI._refreshEventUI  = refreshEventUI
	UI._refreshAfkUI    = refreshAfkUI
...
UI.whTimBtn.MouseButton1Click:Connect(function()
	if S.whTimer then
		stopWHTimer(); tog(UI.whTimBtn, false, C.WH, C.BTN_OFF, "WH Timer: On", "WH Timer: Off"); showN("WH Timer","Disabled",C.ERR)
	else
...
UI.hopNowBtn.MouseButton1Click:Connect(function()
	...
	task.spawn(function()
		local cb=getStat("Beli") or 0; local cf=getStat("Fragments") or 0
		local jt=S.plrC[lp.UserId] and S.plrC[lp.UserId].join or tick()
		S.hopTotal+=1; addLog("Instant Hop")
	...
	if not gp and i.KeyCode == Enum.KeyCode.RightControl then
		...
		if _vis then
			...
			for _, obj in ipairs(full:GetDescendants()) do
				pcall(function()
					if obj:IsA("TextLabel") or obj:IsA("TextButton") then
						TS:Create(obj, TweenInfo.new(.25), { TextTransparency=0 }):Play()
					end
				end)
			end
		else
			...
			for _, obj in ipairs(full:GetDescendants()) do
				pcall(function()
					if obj:IsA("TextLabel") or obj:IsA("TextButton") then
						TS:Create(obj, TweenInfo.new(.2), { TextTransparency=1 }):Play()
					end
				end)
			end
...
local function applyHL(char)
	...
	S.selfHL = nil
	if not char then return end
	S.selfHL = mk("Highlight", char, {
		Name = "ESP_SelfHL",
		FillColor = Color3.new(1,1,1), OutlineColor = Color3.new(0,0,0),
		FillTransparency = .5, OutlineTransparency = 0,
		DepthMode = Enum.HighlightDepthMode.AlwaysOnTop, Adornee = char,
	})
end
...
local function updateFast()
	...
	setText(UI.fpsLbl, "FPS "..S.fps.." / "..S.maxFps)
	...
	if BM2.on then
		...
		if BM2.resetInterval > 0 then
			...
			setText(UI.bm2ResetLbl, "V2 Reset in: "..left.."s")
			setCol(UI.bm2ResetLbl, left < 5 and C.WRN or C.DIM)
		else
			setText(UI.bm2ResetLbl, "V2 Reset: never"); setCol(UI.bm2ResetLbl, C.DIM)
		end
	end
...
local function updateStats()
	...
	if S.sessOK then
		local gb = math.floor((beli or 0) - S.sessB)
		local gf = math.floor((frags or 0) - S.sessF)
...
local function updateRates()
	local bPM2 = calcRate(S.beliHist)
	local fPM2 = calcRate(S.fragHist)
	local function rs(v)
		local sg = v >= 0 and "+" or ""
		return math.abs(v) >= 1e6 and sg..("%.1fM"):format(v/1e6)
			or math.abs(v) >= 1e3 and sg..("%.1fK"):format(v/1e3)
			or sg..tostring(v)
	end
...
local function updateInv()
	local en, elv = getEquipped()
	setText(UI.eqNameLbl, en)
	if elv ~= nil then
		setText(UI.eqLvLbl, "LV "..fmtN(elv)); setCol(UI.eqLvLbl, C.WRN)
	else
		setText(UI.eqLvLbl, en ~= "None" and "No Level" or ""); setCol(UI.eqLvLbl, C.DIM)
	end
	local items = getInv()
	for i = 1, 20 do
		local pf   = UI.invRows[i]
		local item = items[i]
		if item then
			pf.cell.Visible = true
			setText(pf.nameLbl, item.name)
			...
			local rl  = getSkillLevels(item.name)
			local idx = 0
			for _, key in ipairs(SKILL_KEYS) do
				local r = rl[key]
				if r ~= nil and item.level ~= nil then
					local sl = pf.skillLbls[key]
					sl.kl.Position = UDim2.new(0, 8+idx*40, 0, 28)
					sl.cl.Position = UDim2.new(0, 8+idx*40, 0, 39)
					sl.kl.Visible  = true; sl.cl.Visible = true
					setText(sl.cl,   item.level >= r and "OK" or "NO")
					setCol(sl.cl,    item.level >= r and C.OK or C.ERR)
					idx = idx + 1
				end
			end
		else
...
local function updatePlayers()
	...
	table.sort(list, function(a, b)
		if a == lp then return true end; if b == lp then return false end
	...
	for i = 1, 20 do
		...
		if p and pf then
			...
			pf.specBtn.Text   = isSpec and "STOP" or "SPEC"
			pf.specBtn.Active = (p ~= lp)
			if p ~= lp then
				local cache = S.plrC[p.UserId] or {}
		...
		elseif pf then
			pf.row.Visible = false; UI.plrRowMap[i] = nil
		end
	end
end
...
Run.Heartbeat:Connect(function()
	...
	if _frame % 30 == 0 and (S.activeTab == "log" or S.dirty.log) then
		...
		if UI._refreshAfkUI    then UI._refreshAfkUI()    end
		if UI._refreshHealthUI then UI._refreshHealthUI() end
		S.dirty.log = false
	end
end)
...
Pl.PlayerAdded:Connect(function(p)
	...
	addEvent("🟢 Joined", p.Name.." (LV "..(getStat("Level",p) and fmtN(getStat("Level",p)) or "?")..")", C.OK)
	S.dirty.players = true
end)

Pl.PlayerRemoving:Connect(function(p)
	local uid = p.UserId
	local dn  = p.DisplayName ~= p.Name and (p.DisplayName.." (@"..p.Name..")") or p.Name
	showN(dn, "Left the server", C.ERR)
	addEvent("🔴 Left", p.Name, C.ERR)
	if S.specTarget == p then
		stopSpec(); setText(UI.specStatusLbl,"Not spectating"); setCol(UI.specStatusLbl,C.DIM)
		tw(UI.specStopBtn,{BackgroundColor3=C.BTN_OFF},.15); UI.specStopBtn.TextColor3=C.DIM
	end
...
if cfg.HideEnemies then
	task.spawn(function()
		task.wait(2); toggleHidEnm(true)
		tog(UI.enmBtn, true, C.ERR, C.BTN_OFF, "Hide Enemies: On", "Hide Enemies: Off")
	end)
end
...
if cfg.LockFps.on    then
	pcall(function() settings().Rendering.FrameRateManager.MaxFrameRate = cfg.LockFps.fps end)
	pcall(function() setfpscap(cfg.LockFps.fps) end)
end
if cfg.AutoRerun and cfg.AutoRerunURL and cfg.AutoRerunURL ~= "" then
	task.spawn(function() task.wait(3); startRerun(); tog(UI.rerunBtn,true,C.RERUN,C.BTN_OFF,"Auto Rerun: On","Auto Rerun: Off") end)
end

addEvent("🚀 Panel Loaded", "BloxHub v3 Improved started", C.OK)
switchTab("status")
_closeLoader()
