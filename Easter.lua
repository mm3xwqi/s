...
local function deliverFallingSkyEgg(eggInHand)
    ...
    -- Hover above current position and drop the egg
    local hoverY = hrp.Position.Y + 200
    ...
    -- Track the falling egg in-world and follow it
    local trackStart = tick()
    local trackTimeout = 20

    Notification.new({ Title = "Quest", Description = "Tracking Falling Sky Egg...", Duration = 3, Icon = "rbxassetid://8997385628" })

    while _G.AutoFarmEnabled and _G.QuestModeEnabled and tick() - trackStart < trackTimeout do
        -- Find the falling egg in workspace
        local fallingEgg = nil
        for _, v in ipairs(workspace:GetChildren()) do
            if v.Parent and v:IsA("Model") and (v:FindFirstChild("indra egg") or v:FindFirstChild("_PrimaryPart")) then
                fallingEgg = v
                break
        ...
        if fallingEgg then
            -- Follow it while it falls
            local followStart = tick()
            while _G.AutoFarmEnabled and fallingEgg and fallingEgg.Parent and tick() - followStart < 15 do
                local eggPos = getPosition(fallingEgg)
                if not eggPos then break end

                -- Update hover position to track egg horizontally, stay at hoverY
                local direction = Vector3.new(eggPos.X - hrp.Position.X, 0, eggPos.Z - hrp.Position.Z)
            ...
            -- Egg landed or gone — try to collect it
            if fallingEgg and fallingEgg.Parent then
                Notification.new({ Title = "Quest", Description = "Collecting Falling Sky Egg", Duration = 2, Icon = "rbxassetid://8997385628" })
                hoverMoveTo(fallingEgg, hoverY, true, true)
            end
            break
        end
...
local function deliverEggAndWaitForEgg(targetPos, eggInHand, optionButton)
    ...
    if eggSpawned then
        ...
        if closestEgg then
            ...
            hoverMoveTo(closestEgg, hoverY, true, true)
        end
    end
    ...
    if targetSpawned then
        ...
        local function collectWhileHovering()
            ...
            if closestShard then
                ...
                local success = hoverMoveTo(closestShard, hoverY, true, true)
                if success then _G.ShardBlacklist[closestShard] = true end
                return true
            end
            ...
            for _, v in ipairs(workspace:GetChildren()) do
                if v.Parent and v:IsA("Model") and (v:FindFirstChild("indra egg") or v:FindFirstChild("_PrimaryPart")) then
                    ...
                    hoverMoveTo(v, hoverY, true, true)
                    return true
                end
            end
            ...
            if folder then
                ...
                if chestTarget then
                    hoverMoveTo(chestPos, hoverY, true, true)
            ...
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj.Parent and (obj.Name == "EasterChest" or obj.Name == "Chest") and not _G.ChestBlacklist[obj] then
                    ...
                    if p then
                        local pos = getPosition(p)
                        if pos then
                            hoverMoveTo(pos, hoverY, true, true)
        ...
        -- ลูปใหม่: ตรวจสอบและรอไข่เกิดหลังจากเก็บ Shard ครบ
        local idleWaitTime = 0
        while _G.AutoFarmEnabled do
            local collected = collectWhileHovering()
            if collected then
                idleWaitTime = 0 -- รีเซ็ตเวลารอกลับเป็น 0 เมื่อเก็บไอเทมได้
            else
                idleWaitTime = idleWaitTime + 0.1
                -- รอสูงสุด 15 วินาที หลังจากไม่มีไอเทมให้เก็บแล้ว (เพื่อรอ Firefly Egg)
                if idleWaitTime >= 15 then
                    break
                end
            end
            task.wait(0.1)
        ...
        while _G.AutoFarmEnabled do
            ...
            if _G.QuestModeEnabled and eggInHand then
                ...
                elseif string.find(eggInHand.Name, "Falling") then
                    -- FIX: use dedicated falling egg handler that tracks+follows the egg
            ...
            if result == "target_found" then
                while _G.AutoFarmEnabled and collectTargets() do task.wait(0.05) end
                _G.CurrentCircleIndex = _G.CurrentCircleIndex + 1
