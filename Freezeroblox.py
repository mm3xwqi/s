...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            req = urllib.request.Request(GITHUB_API, headers={"User-Agent": "FreezeSwitch"})
...
class Downloader(QThread):
    ...
    def run(self):
        try:
            tmp = tempfile.mktemp(suffix=".exe", prefix="FreezeSwitch_new_")
            req = urllib.request.Request(self.url, headers={"User-Agent": "FreezeSwitch"})
    ...
    def on_no():
        if not silent:
            QMessageBox.information(parent, "Up to date",
                f"You are on the latest version (v{CURRENT_VERSION})!")
...
def main():
    ...
    window.setWindowTitle(f"FreezeSwitch  •  v{CURRENT_VERSION}")
    ...
    key_row.addWidget(key_lbl); key_row.addWidget(key_input); key_row.addWidget(set_key_btn)
    ...
    cooldown.setDecimals(1); cooldown.setValue(0.0); cooldown.setFont(base_font)
    ...
    root.addWidget(freeze_btn); root.addLayout(key_row)
    root.addWidget(cd_lbl); root.addWidget(cooldown)
    root.addWidget(settings_btn); root.addWidget(sbox)
