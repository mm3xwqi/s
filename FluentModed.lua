...
local aa = {
    ...
    [26] = function()
        ...
        function c.New(d, e, f)
            ...
            local function makeValueInput(props)
                local box =
                    ai(
                    "TextBox",
                    {
                        ...
                        TextEditable = true,
                        ClearTextOnFocus = false,
                        ThemeTag = {TextColor3 = "SubText", BackgroundColor3 = "Input"}
                    }
                )
                for key, val in pairs(props) do
                    box[key] = val
                end
                local stroke = ai("UIStroke", {Thickness = 1, Transparency = 1, ThemeTag = {Color = "Accent"}, Parent = box})
                ah.AddSignal(
                    box.Focused,
                    function()
                        isEditing = true
                ...
                ah.AddSignal(
                    ...
                    function()
                        ...
                        if n then
                            ...
                            box.Text = tostring(h.Value)
                        end
                    end
                )
                return box
            end
            local valueInput = makeValueInput({
            ...
            local function setupStackedRailRow()
                ...
                stackedValLabel = makeValueInput({
                    TextSize = 12,
                    TextXAlignment = Enum.TextXAlignment.Left,
            ...
            function h.SetValue(p, s)
                ...
                if not isEditing then
                    valueInput.Text = tostring(p.Value)
                end
                g:SafeCallback(h.Callback, p.Value)
                g:SafeCallback(h.Changed, p.Value)
                if stackedValLabel and not isEditing then
